...
socket.on('connect', () => {
    ...
    if (userJson && userJson !== 'undefined') {  // Check if userJson is not 'undefined'
        try {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const postLink = document.getElementById('Posts'); // Adjust the ID as per your actual HTML

    if (userInfo && userInfo.token) {
        // User is logged in, update UI accordingly
        if (loginLogoutIcon) {
            loginLogoutIcon.innerHTML = '<i class="fa-solid fa-right-from-bracket"></i>';
            loginLogoutIcon.href = 'javascript:logout();'; // Ensure this calls your logout function
        }
        ...
        if (loginLogoutIcon) {
            ...
            loginLogoutIcon.href = 'loginSignup.html'; // Redirect to login/signup page
        }
...
function getUserDataFromLocalStorage() {
    ...
    console.log('User data in local storage:', userData);  // Corrected variable name
    if (userData) {
        try {
